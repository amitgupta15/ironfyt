...
mongodb.MongoClient.connect(process.env.MONGODB_URI, { useNewUrlParser: true, useUnifiedTopology: true }, function (error, database) {
  ...
  //       log.oldmovements = log.movements;
  //       delete log.movements;
  //       db.collection('logs').replaceOne({ _id: log._id }, log, function (error, response) {
  ...
  db.collection('logs')
    .aggregate([
      { $match: { $and: [({ workout_id: { $exists: true } }, { workout_id: { $ne: null } })] } },
      {
    ...
    .toArray((error, workoutlogs) => {
      workoutlogs.forEach((workoutlog) => {
        workoutlog.movements = workoutlog.workout[0].movements;
        db.collection('logs').replaceOne({ _id: workoutlog._id }, workoutlog, function (error, response) {
          console.log(response);
        });
      });
    });
