(function () {
  ...
  $test.it('should enable save button when name and description are added to the workout', function () {
    ...
    $test.assert(selector.innerHTML.includes('<button type="button" id="workout-form-helper-save-new-workout-btn" class="submit-btn" disabled="">Save Workout</button>'));

    // Do something
    document.querySelector('#workout-name').value = 'Workout1';
    document.querySelector('#workout-description').value = 'some description';
    //Dispatch input event from workout-name
    $test.dispatchHTMLEvent('input', '#workout-name');
    //Save button should now be enabled
    $test.assert(selector.innerHTML.includes('<button type="button" id="workout-form-helper-save-new-workout-btn" class="submit-btn">Save Workout</button>'));

    document.querySelector('#workout-name').value = '';
    document.querySelector('#workout-description').value = '';
    $test.dispatchHTMLEvent('input', '#workout-description');
    $test.assert(selector.innerHTML.includes('<button type="button" id="workout-form-helper-save-new-workout-btn" class="submit-btn" disabled="">Save Workout</button>'));

    document.querySelector('#workout-name').value = 'workout 2';
    document.querySelector('#workout-description').value = 'some desc';
    $test.dispatchHTMLEvent('input', '#workout-description');
    $test.assert(selector.innerHTML.includes('<button type="button" id="workout-form-helper-save-new-workout-btn" class="submit-btn">Save Workout</button>'));
  });
